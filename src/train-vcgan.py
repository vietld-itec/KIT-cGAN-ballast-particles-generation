                ...
                if epoch % args.n_critic == 0:
                    ...
                    if epoch % 1000 == 0:#with torch.no_grad():
                        with torch.no_grad():
                            ...
                            chrttitle = f"Epoch:{epoch}, D_loss: {epoch_d_loss:.4f}, G_loss: {epoch_g_loss:.4f}, RMSE: {epoch_rmse:.2f}%"
...
folder_path = f'viet-cgan-{nsamples}-resume'

args.path_file = f'csvdat{nsamples}plus.csv'
...
with open(main_csv_file, 'a', newline='') as csvfile:
    ...
    count = 0
    IsRun = False
    for g_index, g_hid_value in enumerate(g_hid):
        for d_index, d_hid_value in enumerate(d_hid):
            for lr_index, lr_value in enumerate(lr_range):
                # if g_index == 7 and d_index == 1 and lr_index == 5:
                    # print(f"TRUE")
                    # IsRun = True
                if lr_index < 2:
                    continue
                args.batch_size = batch_size
                count += 1
                print(f"{count},gen_hid:{g_hid_value},dis_hid:{d_hid_value}, learn rate:{lr_value}")
                # if IsRun == False:
                    # continue
                ...
                # Save the generator and discriminator models along with input and output dimensions
                #model_data = {
                #    'generator_state_dict': final_generator.state_dict(),
                #    'discriminator_state_dict': final_discriminator.state_dict()
                #}

                #torch.save(model_data, f'{file_name}.pth')
                #print(f"Pretrained models saved:{file_name}")
                ...
                with torch.no_grad():
                    ...
                    min_round = 0.17
                    max_round = 0.25
                    random_tensor = torch.rand(args.batch_size,1)

                    # Scale the values to be between 0.22 and 0.25
                    required_roundness = (max_round - min_round) * random_tensor + min_round


                    noise = generate_noise(args) # Generate noise for 16 samples
                    best_generator.eval()
                    fake_data = best_generator(noise,desired_label,required_roundness) # Generate fake data
                    fake_data = fake_data.cpu().numpy() # Convert to numpy arrays
                    #fake_data = fake_data.tolist() #fake_data.cpu().numpy()# torch.tensor(fake_data, device=device)#cpu().fake_data.numpy().argmax()#np.array(fake_data.detach().cpu().numpy())
                    print(f"fake_data: {np.shape(fake_data )}")
                    #plot_polygons(fake_data, f"", 10, 10) # Plot the polygons(fake_data, epoch) # Plot the polygons
                    plot_save_polygons(fake_data, f"", nrow, ncol, save_path=f"{file_name}/final_class_{classname}.png", dpi=500)
                    np.savetxt(f'{file_name}_class_{classname}_gendata.csv', fake_data, delimiter=',')
                    np.savetxt(f'{file_name}_class_{classname}_request_roundness.csv', required_roundness, delimiter=',')
                    print(f"Fake data saved to {file_name}_gendata.csv.")
